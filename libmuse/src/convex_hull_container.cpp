/**
 * \file      convex_hull_container.cpp
...
namespace muse {
    namespace aggregators {
        ...
        convex_hull_container::convex_hull_container(const std::string & matching_regex, libkerat::slot_t container_slot, bool container_cascade)
        ...
        convex_hull_container::~convex_hull_container(){
            regfree(&m_regex);
        }

        bool convex_hull_container::inside_hull(const libkerat::message::convex_hull& hull, const libkerat::helpers::point_2d& point){
            
            switch (muse::internals::inside_convex_polygonal_container(hull.get_hull(), point)){
                case 0: {
                    return true;
                }
                case 1: {
                ...
                default: {
                    std::cerr << "MUSE:convex_hull_container: Error has occured, see message above. Hull dumped:" << std::endl;
        ...
        int convex_hull_container::process_container_frame(const libkerat::bundle_handle& to_process){
            //clear_hulls();

            // store conves_hull messages
            typedef libkerat::bundle_handle::const_iterator iterator;
            for (iterator i = to_process.begin(); i != to_process.end(); i++){
                libkerat::message::convex_hull * msg_hull = dynamic_cast<libkerat::message::convex_hull*>(*i);
                if (msg_hull != NULL){
                    libkerat::session_id_t sid = msg_hull->get_session_id();
                    hull_map::iterator hull = m_hulls.find(sid);

                    if (hull != m_hulls.end()){
                        hull->second = *msg_hull;
                    } else {
                        m_hulls.insert(hull_map::value_type(sid, *msg_hull));
                    }
        ...
        int convex_hull_container::process_normal_frame(const libkerat::bundle_handle& to_process, libkerat::bundle_handle& output_frame){
        ...
        void convex_hull_container::process_coa(const libkerat::message::container_association& coa, libkerat::bundle_handle& output_frame){
        ...
        void convex_hull_container::process_ala(const libkerat::message::alive_associations& alive_assoc){
        ...
        void convex_hull_container::process_alive(const libkerat::message::alive& alive, libkerat::bundle_handle & output_frame){
                    ...
                    if (!m_cascade){
                        for (sid_coa_map::iterator prev_bnd = tmp_containers.begin(); !(is_inside|record_found) && (tmp_containers.end() != prev_bnd); prev_bnd++){
                            ...
                            if (coa_assoc.find(pt->first) != coa_assoc.end()){
                                record_found = true;
// commented out due to experiment - lock on and not let out
//                                if (inside_hull(m_hulls[prev_bnd->first], pt->second)){
//                                    is_inside = true;
//                                } else {
...
//                                }
                    ...
                    for (hull_map::const_iterator i = m_hulls.begin(); (!is_inside) && (i != m_hulls.end()); i++){
                        if (cid_map->first == i->first){ continue; }
                        is_inside = inside_hull(i->second, pt->second);
                        if (is_inside){
        ...
        int convex_hull_container::process_bundle(const libkerat::bundle_handle & to_process, libkerat::bundle_handle & output_frame){
        ...
        bool convex_hull_container::is_bundle_matching(const libkerat::bundle_handle& bundle){
        ...
        void convex_hull_container::notify(const libkerat::client * cl){
        ...
        void convex_hull_container::purge(){ bm_stack_clear(m_processed_frames); }

        bool convex_hull_container::load(int count){
            return libkerat::adaptor::load(count);
        }

        bool convex_hull_container::load(int count, struct timespec timeout){
